...
_TS_STOPWORDS = set("的了是有在与和及等了吗呢吧啊之其为对于由从而被把让向往就都也很更最才只不没未它他她我们你们他们这个那个这些那些什么怎么怎样如何为什么因为所以但是可是然而如果虽然因此于是然后接着并且而且以及还是此外另外通过根据按照这样那样也就是说例如比如以上以下其中当时首先然后最后正如接着同时不过否则尽管不仅不但除非既然万一倘若何至于是否不能可以就是也是都是也就是说")

def to_query(text: str, user_words: List[str] = None):
    tokenizer = _build_tokenizer(user_words) if user_words else jieba
    extract_tags = [t.strip() for t in tokenizer.lcut(text, cut_all=True)
                    if len(t.strip()) >= 2 and t.strip() not in _TS_STOPWORDS]
